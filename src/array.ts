 ...
 */
export const TcArray = TcClass.new(ArrayStruct, {
  create: Fn.new(
    ArrayStruct.pointer(),
    "tc_array_create",
    [
      ...
      return [
        ...
        allocSize
          ...
          .then([
            ...
            initial_capacity.assign(allocSize.div(element_size)),
          ]),
        // Allocate Array struct
        arr.init(stdlib.malloc(ArrayStruct.sizeOf())),
        arr.notReturn(NULL),
        // Allocate aligned data
        arr._.data.assign(stdlib.aligned_alloc(alignment, allocSize)),
        arr._.data.not().then([
          // Fallback to malloc
          arr._.data.assign(stdlib.malloc(allocSize)),
          // malloc didnt work
        ...
        arr._.length.gte(arr._.capacity).then([
          ...
          newSize
            ...
            .then([
              ...
              newCapacity.assign(newSize.div(arr._.element_size)),
            ]),
          // Allocate new aligned block
          newData.init(stdlib.aligned_alloc(arr._.alignment, newSize)),
          newData.notThen([
            // Fallback to malloc
            newData.assign(stdlib.malloc(newSize)),
            newData.notReturn(-1),
          ]),
          // Copy existing data
          stdstring.memcpy(
          ...
          // Update array
          arr._.data.assign(newData),
          arr._.capacity.assign(newCapacity),
        ]),
      ];
    }
  ),
